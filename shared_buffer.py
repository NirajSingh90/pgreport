import re
import sys
#flowchart 1 setting shared memory
ljust_val = 40
kd = 3
...
def setting_shared_buffer():
    ...
    d = p[0]
    print "#Exctracting OS instruction length"
    if '64' in d:
        os_inst = "64bit"
    else:
        os_inst = "32bit"
    ...
    print "#Exctracting current value os shared_buffer from conf file"
    k = re.findall("shared_buffers\s+=\s+(\d+)MB",postgre_conf)
    current_allocation = k[0]
#    print("Current Value  = " + current_allocation + " MB")
    f_re.write(str("shb:Current_Value").ljust(ljust_val)+"=".ljust(kd)+current_allocation + " MB")
    f_re.write("\n")
    print "#exctracting Total RAM Value"
    p = re.findall(r'Total\sRAM\s+:\s+(\w+)',file)
    actual_ram = p[0]
#    print "System ram  = " + str(int(actual_ram)/1024) + " MB"
#    f_re.write("System ram  = " + str(int(actual_ram)/1024) + " MB")
#    f_re.write("\n")
    print "#Calculating Recommended value of Shared buffer"
...
#        print "Recommended Value".ljust(ljust_val) + str(recommended_ram/1024) + " MB"
 ...
 #       print "Recommended Value".ljust(ljust_val)+"=".ljust(kd) + str(recommended_ram/1024) + " MB"
