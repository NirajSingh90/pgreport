...
def get_os_disk():
    ...
    file = f.read()
    f.close()
    # /dev/sda1                       :  13828095  :  Linux     :  INSTALLATION PARTITION
...
#    print(free_disk)
    ...
    f_re.write("-"*70)
    f_re.write("\n\n")
#    print("OS installation disk partition".ljust(ljust_val)+"=".ljust(kd)+os_disk)
#    print
    f_re.write("OS installation disk partition".ljust(ljust_val)+"=".ljust(kd)+os_disk)
    f_re.write("\n")
#    f_re.write()
#    print "Remaining disks".ljust(ljust_val)+"=".ljust(kd)+str(cp_disks)
#    print
    f_re.write("Remaining disks".ljust(ljust_val)+"=".ljust(kd)+str(cp_disks))
    f_re.write("\n")

    pgxlog_space = calculate_pgxlog_Space()
#    print "PGXLOG space".ljust(ljust_val)+"=".ljust(kd)+str(pgxlog_space)+" MB"
#    print
...
#    del sorted_free_disk['/dev/sda']
    sorted_free_disk.pop()
#    print("rushikesh",sorted_free_disk)
    if len(cp_disks) >= 2:
        flag = 0
        ...
        if flag == 0:
            ...
            #    print "Put WAL to %s"%os_disk
            #    print
                f_re.write("Put WAL ON".ljust(ljust_val)+"=".ljust(kd)+os_disk)
                f_re.write("\n")

            else:
 #               print "Recommendation to add extra disk with size more than " + str(int(pgxlog_space)+100)
 #               print
                f_re.write("Recommendation to add extra disk with size more than " + str(int(pgxlog_space)+100))
                f_re.write("\n")

        else:
#            print "Put WAL ON disk".ljust(ljust_val)+"=".ljust(kd)+xlog_disk
...
#            print
        ...
        if len(cp_disks)%2 == 0:
            #even disks
#            print "Put TEMP  on   :",sorted_free_disk[0][0]
 ...
 #           print "Put DB on    ",ls
            ...
            #odd
            if len(free_disk) == 1:
#                print "Put temp on".ljust(ljust_val)+"=".ljust(kd)+os_disk
#                print "Put db on      :",free_disk
            ...
            #even
#                print "Put temp on    :",sorted_free_disk[0][0]
#                print "Put db on      :",sorted_free_disk[1][0]
                f_re.write("Put TEMP on".ljust(ljust_val)+"=".ljust(kd)+str(sorted_free_disk[0][0]))
                f_re.write("\n")
...
#                print "OS + TEMP + WAL ON DISK".ljust(ljust_val)+"=".ljust(kd)+os_disk
#                print
#                print "DB ON".ljust(ljust_val)+"=".ljust(kd)+cp_disks[0]
#                print
              ...
              #  print "OS + WAL ON DISK".ljust(ljust_val)+"=".ljust(kd)+os_disk
              #  print
              #  print "DB + TEMP ON".ljust(ljust_val)+"=".ljust(kd)+cp_disks[0]
              #  print
            ...
            if pgxlog_space >= free_disk[os_disk]:
               # print "Recommendation to add extra disk of size greater than ".ljust(ljust_val)+"=".ljust(kd)+ str(pgxlog_space)
               # print "OS + WAL + TEMP + DB ON DISK".ljust(ljust_val)+"=".ljust(kd)+os_disk
               # print
               ...
               # print "OS + WAL + TEMP + DB ON DISK         :",os_disk
               # print
                f_re.write("OS + WAL + TEMP + DB ON DISK".ljust(ljust_val)+"=".ljust(kd)+os_disk)
                f_re.write("\n")
